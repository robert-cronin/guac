...
func TestDataDogCertifier_CertifyComponent(t *testing.T) {
	...
	tests := []struct {
		...
		expectedPreds  int
		wantPackages   []string // List of package names we expect to find
		wantErr        bool
		errMessage     error
	...
	}{
		{
			...
			rootComponent: []*root_package.PackageNode{
				...
				{Purl: "pkg:maven/safe/package@1.0.0"}, // Should be ignored
			},
			expectedPreds: 3,
			wantPackages: []string{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			certifier, err := NewDataDogCertificationParser(ctx, mockAssembler, WithHTTPClient(testClient))
			if err != nil {
...
func (t *mockTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	// Create a new URL that points to our test server but keeps the path
	newURL := t.server.URL
	...
	// Use the test server's client to do the actual request
